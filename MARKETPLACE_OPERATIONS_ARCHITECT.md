...
> [!IMPORTANT]
> **TARGET ARCHITECTURE DISCLAIMER (NOT IMPLEMENTED):**
> The current Kazify application runs on a unified Express + React + Vite runtime. References in this document to NestJS backend modules, NestJS controllers, or microservice deployments represent **Target Architecture (not implemented)** for future enterprise scale-out.

---

## 1. Dashboard & Core Architecture

The Kazify Admin Control Center is designed to operate on top of an Express backend (with NestJS targeted for future microservices scale-out). It provides operational visibility, compliance tooling, and financial auditing while strictly segregating administrative functions from customer and expert domains.
...
### 4.1 REST API Controller Routes (Target NestJS Specification - Not Implemented)
