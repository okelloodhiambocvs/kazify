...
export default function BookingCalendar({ fundiId, isEditable, onDateSelected, selectedDate }: BookingCalendarProps) {
  ...
  return (
    <div className="p-5 bg-slate-950/80 border border-slate-800/80 rounded-2xl space-y-4 shadow-xl" id={`booking-calendar-${fundiId}`}>
      ...
      {success && (
        <div className="p-2.5 bg-sky-500/10 border border-sky-500/20 rounded-xl text-[10px] text-sky-400 font-mono flex items-center gap-1.5 animate-in fade-in">
          ...
          {!isEditable && (
            <div className="p-3 bg-slate-900/50 border border-slate-800 rounded-xl flex items-start gap-2">
              ...
              <div className="space-y-0.5 text-[9px] font-mono text-slate-400 leading-relaxed">
                ...
                <span>Select an available day styled in <strong className="text-sky-400 font-bold">Light Blue</strong> on the calendar, then propose an appointment slot directly in client chats!</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="md:col-span-7 space-y-3">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              if (isEditable) {
                ...
                if (isBlocked) {
                  ...
                  cellClass = "bg-sky-500/5 border border-sky-500/20 text-sky-400 hover:bg-sky-500/10 hover:border-sky-500 cursor-pointer relative";
                  badgeElement = <span className="absolute bottom-0.5 right-0.5 text-[6px] font-mono text-sky-400 font-bold uppercase">Ready</span>;
                } else {
                ...
                if (isSelected) {
                  ...
                  cellClass = "bg-sky-500/10 border border-sky-500/30 text-sky-400 hover:bg-sky-500/15 hover:border-sky-400 cursor-pointer";
                } else {
                  cellClass = "bg-slate-950 border border-slate-900 text-slate-700 cursor-not-allowed opacity-40 flex items-center justify-center relative";
          ...
          <div className="flex flex-wrap items-center justify-start gap-4 pt-2 text-[9px] font-mono text-slate-500">
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded bg-sky-500/20 border border-sky-500/40 block" />
