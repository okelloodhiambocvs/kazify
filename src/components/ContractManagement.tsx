...
export default function ContractManagement({ user, onContractSigned }: ContractManagementProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active':
        return <span className="text-[10px] font-bold bg-sky-500/10 border border-sky-500/20 text-sky-400 px-2.5 py-0.5 rounded-full uppercase tracking-wider">● ACTIVE</span>;
      case 'completed':
        ...
        <div className="lg:col-span-7">
          {selectedContract ? (
            <div className="space-y-4">
              ...
              {negotiationSuccess && (
                <div className="p-3 bg-sky-500/5 border border-sky-500/15 rounded-xl text-sky-400 text-xs font-mono flex items-center gap-1.5 animate-in fade-in">
                  <CheckCircle2 className="w-4.5 h-4.5 text-sky-400" />
