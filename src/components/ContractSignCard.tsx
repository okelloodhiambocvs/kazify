    ...
    <div className="border border-slate-800 rounded-2xl bg-slate-950 p-5 text-left space-y-4">
      ...
      <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-950 space-y-2 text-xs font-mono">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-white font-bold">KES {(contract?.amount || 0).toLocaleString()}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-slate-500">Platform Commission Fee (10%):</span>
          <span className="text-rose-400 font-bold">-KES {(commission || 0).toLocaleString()}</span>
        </div>
        <div className="border-t border-slate-900 my-1 pt-1.5 flex justify-between font-bold">
          ...
          <span className="text-sky-400">KES {(payout || 0).toLocaleString()}</span>
        </div>
      </div>

      {/* Signature States */}
      <div className="grid grid-cols-2 gap-3 pt-1">
        <div className={`p-3 rounded-xl border flex items-center gap-2.5 text-xs font-mono ${contract.customer_signed ? 'bg-sky-500/5 border-sky-500/10 text-sky-400' : 'bg-slate-900/40 border-slate-800 text-slate-500'}`}>
          <CheckCircle2 className={`w-4.5 h-4.5 flex-shrink-0 ${contract.customer_signed ? 'text-sky-400 animate-pulse' : 'text-slate-600'}`} />
        ...
        <div className={`p-3 rounded-xl border flex items-center gap-2.5 text-xs font-mono ${contract.fundi_signed ? 'bg-sky-500/5 border-sky-500/10 text-sky-400' : 'bg-slate-900/40 border-slate-800 text-slate-500'}`}>
          <CheckCircle2 className={`w-4.5 h-4.5 flex-shrink-0 ${contract.fundi_signed ? 'text-sky-400 animate-pulse' : 'text-slate-600'}`} />
          <div className="text-left">
      ...
      {bothSigned ? (
        <div className="p-3 bg-sky-500/5 border border-sky-500/15 rounded-xl text-sky-400 text-xs font-mono flex items-center gap-2">
